<template>
  <div class="page">
    <div ref="Panel" :style="{ height: boxWidth + 'px', width: boxWidth + 'px' }" class="tagsCloud" @mousemove="listener($event)">
      <div
...
<script>
import Vue from 'vue'
import { listAllTags } from '../../../api/posts'
export default {
  ...
  methods: {
    init() {
      ...
      this.useArray.forEach((item, key) => {
        const k = -1 + (2 * (key + 1) - 1) / this.useArray.length
        const a = Math.acos(k)
        const b = a * Math.sqrt(this.useArray.length * Math.PI)
        ...
        // Z-index
        const scale = this.boxWidth / (this.boxWidth - item.z)
