      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions>{{ scope.row.deleted===0?'正常':'已禁用' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="createTime"
        ...
        <el-table-column
          ...
          <template scope="scope">
            ...
            <el-button type="danger" size="small" @click="handleDel(scope)">禁用</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--    编辑表单-->
      <el-dialog :visible.sync="dialogFormVisible" title="编辑">
        <el-form :model="editForm">
          ...
          <el-form-item :label-width="'75px'" label="状态">
            <el-radio-group v-model="editForm.deleted">
...
<script>
import { addRole, deleteBatch, deleteRole, listRole, updateRole } from '../../api/Role'
  ...
  methods: {
    ...
    handleDel(row) {
      this.$confirm('确认删除该角色吗?', '提示', {
        ...
        deleteRole(row.row.id).then(res => {
          if (res.code === 200) {
            this.$message.success(res.msg)
          }
          this.query()
