      ...
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-button
            ...
            icon="el-icon-delete"
            size="small"
            @click="handleDelete">批量禁用</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    <!--列表-->
    <template>
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag
              disable-transitions>{{ scope.row.roleName }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          label="状态"
          ...
          <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions>{{ scope.row.userStatus }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          label="操作"
          ...
          <template scope="scope">
            ...
            <el-button
              ...
              type="danger"
              size="small"
              @click="handleDel(scope,1)">禁用</el-button>
            <el-button
              v-if="scope.row.status!==0"
              type="primary"
              size="small"
              @click="handleDel(scope,0)">恢复</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--    编辑表单-->
      <el-dialog :visible.sync="dialogFormVisible" title="编辑">
        <el-form :model="editForm">
          <el-form-item :label-width="'75px'" label="邮箱">
            <el-input v-model="editForm.email" disabled readonly/>
          </el-form-item>
          <el-form-item :label-width="'75px'" label="昵称">
...
<script>
import { list, sysUpdate } from '../../api/user_info'
...
export default {
  ...
  created() {
    const page = { total: 0, pageSize: 10, currentPage: 1, roleName: '' }
    listRole(page).then(res => {
      this.role = res.data
    })
    this.query()
  },
  methods: {
    ...
    handleDel(row, status) {
      ...
      this.$confirm('确认' + statuText + '该用户吗?', '提示', {
        type: 'warning'
      }).then(() => {
        const Form = {
          id: row.row.id,
