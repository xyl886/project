...
<script>
import { sysUpdate } from '../../api/user_info'
import { deleteBatch, deleteComment, listComment } from '../../api/posts_comment'
...
export default {
  ...
  methods: {
    ...
    query() {
      this.loading = true
      listComment(this.page).then(res => {
        if (res.code === 200) {
    ...
    handleDel(row) {
      this.$confirm('确认删除该评论吗?', '提示', {
        ...
        deleteComment(row.row.id).then(res => {
          if (res.code === 200) {
            this.$message.success(res.msg)
          }
          this.query()
